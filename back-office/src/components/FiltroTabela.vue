        ...
        <div v-if="showFilterModal" class="modal-overlay" @click.self="closeFilter">
            <div class="modal-container">
                <header class="modal-header">
                    <h3>Filtrar</h3>
                    <button class="modal-close" @click="closeFilter" aria-label="Fechar">×</button>
                </header>

                <div class="modal-body">
                    <div v-for="(values, key) in filterOptions" :key="key" class="filter-group">
                        <p class="filter-label">{{ getColumnLabel(key) }}</p>
                        <div class="filter-items">
                            <label v-for="value in values" :key="value" class="filter-item">
                                <input type="checkbox" :value="value" v-model="internalFilters[key]" />
                                {{ value }}
                            </label>
                        </div>
                    </div>
...
watch(() => props.modelSort, val => sortKey.value = val);
watch(sortKey, val => emit('update:modelSort', val));

const sortOrder = ref(props.modelOrder);
watch(() => props.modelOrder, val => sortOrder.value = val);
